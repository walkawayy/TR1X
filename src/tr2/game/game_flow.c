...
static void M_SkipStringTable(VFILE *file, int32_t count);

static void M_SkipStringTable(VFILE *const file, const int32_t count)
{
    VFile_Skip(file, sizeof(int16_t) * count);
...
bool GF_LoadFromFile(const char *const file_name)
{
    ...
    VFile_Skip(file, GF_DESCRIPTION_SIZE);
    ...
    M_SkipStringTable(file, num_levels);
    M_SkipStringTable(file, num_pictures);
    M_SkipStringTable(file, num_titles);
    M_SkipStringTable(file, num_fmvs);
    M_SkipStringTable(file, num_levels);
    M_SkipStringTable(file, num_cutscenes);
