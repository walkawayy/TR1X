...
static void M_SetupCreatures(void)
{
    ...
    BaconLara_Setup(Object_GetObject(O_BACON_LARA));
    Wolf_Setup(Object_GetObject(O_WOLF));
    Bear_Setup(Object_GetObject(O_BEAR));
    Bat_Setup(Object_GetObject(O_BAT));
    TRex_Setup(Object_GetObject(O_TREX));
    Raptor_Setup(Object_GetObject(O_RAPTOR));
    Larson_Setup(Object_GetObject(O_LARSON));
    Pierre_Setup(Object_GetObject(O_PIERRE));
    Rat_Setup(Object_GetObject(O_RAT));
    Vole_Setup(Object_GetObject(O_VOLE));
    Lion_SetupLion(Object_GetObject(O_LION));
    Lion_SetupLioness(Object_GetObject(O_LIONESS));
    Lion_SetupPuma(Object_GetObject(O_PUMA));
    Croc_Setup(Object_GetObject(O_CROCODILE));
    Alligator_Setup(Object_GetObject(O_ALLIGATOR));
    Ape_Setup(Object_GetObject(O_APE));
    Mutant_Setup(Object_GetObject(O_WARRIOR_1));
    Mutant_Setup2(Object_GetObject(O_WARRIOR_2));
    Mutant_Setup3(Object_GetObject(O_WARRIOR_3));
    Centaur_Setup(Object_GetObject(O_CENTAUR));
    Mummy_Setup(Object_GetObject(O_MUMMY));
    SkateKid_Setup(Object_GetObject(O_SKATEKID));
    Cowboy_Setup(Object_GetObject(O_COWBOY));
    Baldy_Setup(Object_GetObject(O_BALDY));
    Torso_Setup(Object_GetObject(O_TORSO));
    Natla_Setup(Object_GetObject(O_NATLA));
    Pod_Setup(Object_GetObject(O_PODS));
    Pod_Setup(Object_GetObject(O_BIG_POD));
    Statue_Setup(Object_GetObject(O_STATUE));
}

static void M_SetupTraps(void)
{
    FallingBlock_Setup(Object_GetObject(O_FALLING_BLOCK));
    Pendulum_Setup(Object_GetObject(O_PENDULUM));
    TeethTrap_Setup(Object_GetObject(O_TEETH_TRAP));
    RollingBall_Setup(Object_GetObject(O_ROLLING_BALL));
    Spikes_Setup(Object_GetObject(O_SPIKES));
    FallingCeiling_Setup(Object_GetObject(O_FALLING_CEILING_1));
    FallingCeiling_Setup(Object_GetObject(O_FALLING_CEILING_2));
    DamoclesSword_Setup(Object_GetObject(O_DAMOCLES_SWORD));
    ...
    SlidingPillar_Setup(Object_GetObject(O_SLIDING_PILLAR));
    LightningEmitter_Setup(Object_GetObject(O_LIGHTNING_EMITTER));
    ThorsHammerHandle_Setup(Object_GetObject(O_THORS_HANDLE));
    ThorsHammerHead_Setup(Object_GetObject(O_THORS_HEAD));
    MidasTouch_Setup(Object_GetObject(O_MIDAS_TOUCH));
    DartEmitter_Setup(Object_GetObject(O_DART_EMITTER));
    Dart_Setup(Object_GetObject(O_DART));
    DartEffect_Setup(Object_GetObject(O_DART_EFFECT));
    FlameEmitter_Setup(Object_GetObject(O_FLAME_EMITTER));
    Flame_Setup(Object_GetObject(O_FLAME));
    EmberEmitter_Setup(Object_GetObject(O_EMBER_EMITTER));
    Ember_Setup(Object_GetObject(O_EMBER));
    LavaWedge_Setup(Object_GetObject(O_LAVA_WEDGE));
}

static void M_SetupMiscObjects(void)
{
    CameraTarget_Setup(Object_GetObject(O_CAMERA_TARGET));
    BridgeFlat_Setup(Object_GetObject(O_BRIDGE_FLAT));
    BridgeTilt1_Setup(Object_GetObject(O_BRIDGE_TILT_1));
    BridgeTilt2_Setup(Object_GetObject(O_BRIDGE_TILT_2));
    Drawbridge_Setup(Object_GetObject(O_DRAWBRIDGE));
    Switch_Setup(Object_GetObject(O_SWITCH_TYPE_1));
    Switch_SetupUW(Object_GetObject(O_SWITCH_TYPE_2));
    ...
    TrapDoor_Setup(Object_GetObject(O_TRAPDOOR_1));
    TrapDoor_Setup(Object_GetObject(O_TRAPDOOR_2));
    Cog_Setup(Object_GetObject(O_COG_1));
    Cog_Setup(Object_GetObject(O_COG_2));
    Cog_Setup(Object_GetObject(O_COG_3));
    MovingBar_Setup(Object_GetObject(O_MOVING_BAR));
    ...
    Pickup_Setup(Object_GetObject(O_SG_AMMO_ITEM));
    Pickup_Setup(Object_GetObject(O_MAG_AMMO_ITEM));
    Pickup_Setup(Object_GetObject(O_UZI_AMMO_ITEM));
    Pickup_Setup(Object_GetObject(O_EXPLOSIVE_ITEM));
    Pickup_Setup(Object_GetObject(O_MEDI_ITEM));
    Pickup_Setup(Object_GetObject(O_BIGMEDI_ITEM));

    Scion1_Setup(Object_GetObject(O_SCION_ITEM_1));
    Scion2_Setup(Object_GetObject(O_SCION_ITEM_2));
    Scion3_Setup(Object_GetObject(O_SCION_ITEM_3));
    Scion4_Setup(Object_GetObject(O_SCION_ITEM_4));
    ScionHolder_Setup(Object_GetObject(O_SCION_HOLDER));

    Pickup_Setup(Object_GetObject(O_LEADBAR_ITEM));
    SaveCrystal_Setup(Object_GetObject(O_SAVEGAME_ITEM));
    ...
    PuzzleHole_Setup(Object_GetObject(O_PUZZLE_HOLE_1));
    PuzzleHole_Setup(Object_GetObject(O_PUZZLE_HOLE_2));
    PuzzleHole_Setup(Object_GetObject(O_PUZZLE_HOLE_3));
    PuzzleHole_Setup(Object_GetObject(O_PUZZLE_HOLE_4));
    PuzzleHole_SetupDone(Object_GetObject(O_PUZZLE_DONE_1));
    PuzzleHole_SetupDone(Object_GetObject(O_PUZZLE_DONE_2));
    PuzzleHole_SetupDone(Object_GetObject(O_PUZZLE_DONE_3));
    PuzzleHole_SetupDone(Object_GetObject(O_PUZZLE_DONE_4));

    Cabin_Setup(Object_GetObject(O_PORTACABIN));
    Boat_Setup(Object_GetObject(O_BOAT));
    Earthquake_Setup(Object_GetObject(O_EARTHQUAKE));
    ...
    Blood_Setup(Object_GetObject(O_BLOOD_1));
    Bubble_Setup(Object_GetObject(O_BUBBLES_1));
    Explosion_Setup(Object_GetObject(O_EXPLOSION_1));
    Ricochet_Setup(Object_GetObject(O_RICOCHET_1));
    Twinkle_Setup(Object_GetObject(O_TWINKLE));
    PickupAid_Setup(Object_GetObject(O_PICKUP_AID));
    Splash_Setup(Object_GetObject(O_SPLASH_1));
    Waterfall_Setup(Object_GetObject(O_WATERFALL));
    BodyPart_Setup(Object_GetObject(O_BODY_PART));
    NatlaGun_Setup(Object_GetObject(O_MISSILE_1));
    Missile_Setup(Object_GetObject(O_MISSILE_2));
    Missile_Setup(Object_GetObject(O_MISSILE_3));
    GunShot_Setup(Object_GetObject(O_GUN_FLASH));
}
